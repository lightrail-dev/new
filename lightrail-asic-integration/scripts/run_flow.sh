#!/usr/bin/env bash
# Full RTL-to-GDSII flow for LightRail × ElemRV-N on IHP SG13G2
...
echo "=== [1/5] task prepare  – RTL generation + bondpad files ==="
task prepare
...
echo "=== [2/5] task layout   – Yosys synthesis + OpenROAD P&R ==="
task layout

echo "=== [3/5] task filler   – filler cell insertion ==="
task filler

echo "=== [4/5] task run-drc  – KLayout DRC against IHP deck ==="
task run-drc
