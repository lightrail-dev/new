...
# Inputs:
#   ElemRV/build/**/*.gds        – ASIC layout (from run_flow.sh)
#   lightrail-pcb/fab/           – PCB Gerbers (KiCad export)
# Output:
#   fab-package-<date>/          – complete fab submission directory
set -euo pipefail
...
# ── BGA pinout cross-reference ────────────────────────────────────────────────
echo "Writing BGA-2500 cross-reference..."
cat > "$OUT/docs/bga2500_pinout_crossref.txt" << 'EOF'
LightRail BGA-2500 ↔ ElemRV-N I/O Cross-Reference
Package: 50×50 array, 0.8 mm pitch, 4.5×4.5 mm die
PDK: IHP SG13G2
...
LightRail × ElemRV ASIC Fab Package
Generated : $DATE
SOC       : ElemRV-N
...
Contents:
  asic/   – GDS (final layout), LEF (abstract), DRC log
  pcb/    – PCB Gerbers (32-layer Rogers 4350B HDI, 420×350 mm)
  docs/   – BGA-2500 ball map cross-reference

Fab submission checklist:
  [ ] DRC clean (zero violations) — verify asic/*.lyrdb
  ...
  [ ] PCIe Gen5 impedance stack-up confirmed (100 Ω diff, Rogers 4350B)
EOF
